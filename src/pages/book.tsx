...
import SubtypePickerModal, {
  type ServiceSubtype,
} from "./../components/SubtypePickerModal";
...
const steps = [
  {
    ...
    name: "Especialista",
    icon: User,
    description: "Selecciona tu especialista preferido",
  },
  {
    id: 2,
    name: "Servicio",
...
// Normaliza especialistas por si el backend envía `name` en vez de `display_name`
function normalizeSpecialists(arr: any[]): Specialist[] {
  return (arr ?? []).map((s: any) => ({
    ...s,
    display_name: s?.display_name ?? s?.name ?? "Sin nombre",
  }));
}
...
export default function PremiumBooking() {
  ...
  const [pickedSubtype, setPickedSubtype] = useState<ServiceSubtype | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((r) => setSpecialists(normalizeSpecialists(r.data.data || [])))
  ...
  const priceToShow =
    pickedSubtype?.price ??
    (selectedService ? selectedService.price : undefined);
  const durationToShow =
    pickedSubtype?.duration_min ??
    (selectedService ? selectedService.duration_min : undefined);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-card to-background">
      <div className="max-w-4xl mx-auto px_4 py-8">
        {/* Header */}
        <motion.div
        ...
        >
          ...
          <p className="text-muted-foreground">
            Programa tu cita de manera fácil y rápida
          </p>
        </motion.div>
          ...
          <div className="relative">
            ...
            <div className="absolute top-6 left-6 right-6 h-0.5 bg-border -z-0">
              <motion.div
                ...
                transition={{ duration: 0.5, ease: [0.42, 0, 0.58, 1] }} // ease-in-out bezier
              />
            </div>
          </div>
        </motion.div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="font-medium text-foreground">
                          {selectedSpecialist.display_name}
                        </h3>
