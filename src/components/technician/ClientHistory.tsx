...
import { Search, FileText, Calendar, DollarSign, ChevronDown, ChevronUp, Package, Lightbulb, Mail, Phone, Users } from 'lucide-react'
...
export default function ClientHistory() {
  ...
  const loadClientRecords = async () => {
    try {
      ...
      if (response.success) {
        // Filtrar solo los clientes que este técnico ha atendido
        const myClients = response.data.filter((record: ClientRecord) => {
          // Un cliente es "mío" si tengo al menos una nota registrada para ellos
          return record.notes.some(note => note.technician_id === user?.id)
        }).map((record: ClientRecord) => {
          // Filtrar las notas para mostrar solo las creadas por este técnico
          return {
            ...record,
            notes: record.notes.filter(note => note.technician_id === user?.id),
            total_notes: record.notes.filter(note => note.technician_id === user?.id).length
          }
        })

        setRecords(myClients)
        setFilteredRecords(myClients)
      }
    } catch (err: any) {
      setError(err.message || 'Error al cargar el historial de clientes')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-neutral-900">Mis Clientes</h2>
          <p className="text-neutral-600 mt-1">
            Historial de {records.length} {records.length === 1 ? 'cliente atendido' : 'clientes atendidos'}
          </p>
        </div>
        ...
        {filteredRecords.length === 0 ? (
          <div className="text-center py-12 bg-neutral-50 rounded-xl">
            ...
            <p className="text-neutral-600 font-medium mb-2">
              {searchTerm ? 'No se encontraron clientes' : 'No has atendido clientes todavía'}
            </p>
            {!searchTerm && (
              <p className="text-sm text-neutral-500">
                Los clientes aparecerán aquí después de completar tu primera cita
              </p>
            )}
          </div>
        ) : (
          filteredRecords.map((record) => {
            ...
            return (
              <div
                ...
                className="border-2 border-neutral-200 rounded-xl overflow-hidden hover:border-neutral-300 transition-all bg-white"
              >
                {/* Client Header */}
                <button
                ...
                >
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="text-left">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center sm:space-x-4 text-sm text-neutral-600 mt-1 gap-1">
                        <span className="flex items-center space-x-1">
                          <Mail className="w-3 h-3" />
                          <span className="truncate max-w-[200px]">{record.client_email}</span>
                        </span>
                        {record.client_phone && (
                          <span className="flex items-center space-x-1">
                  ...
                  <div className="border-t border-neutral-200 bg-neutral-50 p-5 space-y-4 animate-fadeIn">
                    ...
                    <div>
                      <h4 className="font-semibold text-neutral-900 mb-3 flex items-center space-x-2">
                        ...
                        <span>Mis Observaciones ({record.notes.length})</span>
                      </h4>

                      {record.notes.length === 0 ? (
                        <p className="text-center text-neutral-500 py-8 bg-white rounded-lg border border-neutral-200">
    ...
    <div className="bg-white border-2 border-neutral-200 rounded-lg p-4 hover:border-neutral-300 transition-all">
      ...
      <div className="space-y-3">
        <div>
          ...
          <p className="text-sm text-neutral-600 pl-6 whitespace-pre-wrap">{note.observations}</p>
        </div>
        ...
        {note.products_used && (
          <div>
            ...
            <p className="text-sm text-neutral-600 pl-6 whitespace-pre-wrap">{note.products_used}</p>
          </div>
        )}

        {note.recommendations && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
            <p className="text-sm font-semibold text-blue-900 mb-1 flex items-center space-x-2">
              <Lightbulb className="w-4 h-4" />
              <span>Recomendaciones al cliente:</span>
            </p>
            <p className="text-sm text-blue-700 pl-6 whitespace-pre-wrap">{note.recommendations}</p>
          </div>
        )}
      </div>
